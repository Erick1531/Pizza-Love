...
function confirmBranch(branch) {
    ...
    let mensaje = `*NUEVO PEDIDO - Pizza & Love*\n\n`;
    mensaje += `*Sucursal:* ${branch.nombre}\n`;
    mensaje += `*Direccion:* ${branch.direccion}\n\n`;
    mensaje += `--- PEDIDO ---\n`;
    ...
    cart.forEach((item, index) => {
        ...
        mensaje += `   Precio: $${item.totalPrice}\n`;
    });
    
    mensaje += `\n*TOTAL: $${getCartTotal()}*\n\n`;
    mensaje += `Gracias por tu preferencia!`;
    
    // Codificar el mensaje de forma más compatible
    const mensajeCodificado = encodeURIComponent(mensaje);
    
    // Limpiar el número de teléfono
    let phoneNumber = branch.tel.replace(/[\s\-()]/g, '');
    
    // Si el número no empieza con +, agregarlo (asumiendo México +52)
    if (!phoneNumber.startsWith('+')) {
        phoneNumber = '+52' + phoneNumber;
    }
    
    // Crear URL de WhatsApp (usar api.whatsapp.com es más confiable)
    ...
    modalBody.innerHTML = `
        <div class="order-success">
            ...
            <h3 style="color: #10b981; font-size: 1.5rem; margin: 1rem 0;">¡Pedido Realizado con Éxito!</h3>
            <p style="color: #6b7280; margin-bottom: 1.5rem;">
                Tu pedido está listo para ser enviado a la sucursal ${branch.nombre}
            </p>
            <p style="color: #374151; font-weight: 500; margin-bottom: 1rem;">
                Total: $${getCartTotal()}
            </p>
            ...
            <p style="color: #9ca3af; font-size: 0.875rem; margin-top: 1.5rem;">
                ¡Disfruta de tu pedido! 🍕❤️
            </p>
        </div>
    `;
    
    // Limpiar carrito después de 2 segundos (dar tiempo para que se vea el mensaje)
    setTimeout(() => {
        cart = [];
        selectedBranch = null;
        saveCart();
